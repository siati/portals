...
class User extends Component {
    ...
    public function login(IdentityInterface $identity, $duration = 0) {
        if ($this->beforeLogin($identity, false, $duration)) {
            ...
            $id = $identity->getId();
            $identity->user_ip = Yii::$app->getRequest()->getUserIP();
            $identity->session_id = Yii::$app->getSession()->id;
            if ($this->enableSession) {
                $log = "User '$id' logged in from $identity->user_ip with duration $duration.";
            } else {
                $log = "User '$id' logged in from $identity->user_ip. Session not enabled.";
            }
    ...
    protected function afterLogin($identity, $cookieBased, $duration) {
        ...
        if (!$cookieBased) {
            $identity->last_signed_in = \common\models\StaticMethods::now();
    ...
    protected function afterLogout($identity) {
        ...
        $identity->last_signed_out = \common\models\StaticMethods::now();
