...
class Model extends Component implements IteratorAggregate, ArrayAccess, Arrayable {
     ...
     * @param string $attribute attribute of [[Model]]
     */
    public function absoluteValue($attribute) {
        if (!is_numeric($this->$attribute) || $this->$attribute != abs($this->$attribute))
     ...
     */
    public function sanitizeString($attribute) {
        $this->$attribute = str_replace('\\', '', str_replace("'", '', str_replace('"', '', $this->$attribute)));
    }
    ...
    /**
     * postal address, if supplied, must be impartial
     */
    public function fullPostalAddress() {
        if (!empty($this->postal_no) || !empty($this->postal_code))
            if (empty($this->postal_no))
                $this->addError('postal_no', 'Postal No. is required too');
            else
            if (empty($this->postal_code))
                $this->addError('postal_code', 'Postal Code is required too');
    }

    /**
     * employer postal address, if supplied, must be impartial
     */
    public function fullEmployerPostalAddress() {
        if (!empty($this->employer_postal_no) || !empty($this->employer_postal_code))
            if (empty($this->employer_postal_no))
                $this->addError('employer_postal_no', 'Postal No. is required too');
            else
            if (empty($this->employer_postal_code))
                $this->addError('employer_postal_code', 'Postal Code is required too');
    }

    /**
     * 
     * @param string $attribute pin no
     */
    public function KRAPin($attribute) {
        if (!in_array(substr($this->$attribute, 0, 1), range('A', 'Z')) || !in_array(substr($this->$attribute, -1), range('A', 'Z')) || !is_numeric(substr($this->$attribute, 1, 9)))
            $this->addError($attribute, 'Invalid PIN');
    }
