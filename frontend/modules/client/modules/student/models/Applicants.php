...
class Applicants extends \yii\db\ActiveRecord {
    ...
    public function compileProfile() {
        ...
        foreach ($education_backgrounds as $education_background) {
            $education_background->study_level == EducationBackground::study_level_primary && $education_background->sponsored == EducationBackground::sponsored_yes && !is_object(ApplicantSponsors::forApplicantAndStudyLevel($this->id, ApplicantSponsors::study_level_primary . ',' . ApplicantSponsors::study_level_primary_secondary . ',' . ApplicantSponsors::study_level_primary_secondary_tertiary, ApplicantSponsors::one)) ? $profile[self::profile_has_sponsor_primary] = [self::narration => 'Sponsor for primary education missing', self::required => true] : '';
            $education_background->study_level == EducationBackground::study_level_secondary && $education_background->sponsored == EducationBackground::sponsored_yes && !is_object(ApplicantSponsors::forApplicantAndStudyLevel($this->id, ApplicantSponsors::study_level_secondary . ',' . ApplicantSponsors::study_level_primary_secondary . ',' . ApplicantSponsors::study_level_primary_secondary_tertiary, ApplicantSponsors::one)) ? $profile[self::profile_has_sponsor_secondary] = [self::narration => 'Sponsor for secondary education missing', self::required => true] : '';
            !in_array($education_background->study_level, [EducationBackground::study_level_primary, EducationBackground::study_level_secondary]) && $education_background->sponsored == EducationBackground::sponsored_yes && !is_object(ApplicantSponsors::forApplicantAndStudyLevel($this->id, ApplicantSponsors::study_level_tertiary . ',' . ApplicantSponsors::study_level_primary_secondary_tertiary, ApplicantSponsors::one)) ? $profile[self::profile_has_sponsor_tertiary] = [self::narration => 'Sponsor for tertiary education missing', self::required => true] : '';
        }
    ...
    public static function correctionAction($section) {
            ...
            if (in_array($section, [self::profile_has_father, self::profile_has_mother, self::profile_has_guardian, self::profile_has_guardian_to_father, self::profile_has_guardian_to_mother]))
                return "$('#sd-nav-prnts').click()";
