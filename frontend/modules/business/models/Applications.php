...
class Applications extends \yii\db\ActiveRecord {
    ...
    public function compileApplication($is_appeal) {
        if (!$this->applicationOrAppealPrintOutExists($is_appeal))
            if ($is_appeal)
                $profile = Applicants::returnApplicant($this->applicant)->compileProfile();
            else
                foreach (ProductOpeningSettings::forApplicationSettingAndActive($this->application, null, ProductOpeningSettings::active, ProductOpeningSettings::all) as $setting) {
                    ...
                    $setting->setting == ProductSettings::certificate && $setting->value == ProductSettings::yes && empty(EducationBackground::searchEducations($this->applicant, EducationBackground::study_level_certificate)) ? $profile[Applicants::profile_has_education_background_certificate] = [Applicants::narration => 'Tertiary Certificate Missing', Applicants::required => true] : '';
                    $setting->setting == ProductSettings::diploma && $setting->value == ProductSettings::yes && empty(EducationBackground::searchEducations($this->applicant, EducationBackground::study_level_diploma)) ? $profile[Applicants::profile_has_education_background_diploma] = [Applicants::narration => 'Diploma Certificate Missing', Applicants::required => true] : '';
                    $setting->setting == ProductSettings::degree && $setting->value == ProductSettings::yes && empty(EducationBackground::searchEducations($this->applicant, EducationBackground::study_level_degree)) ? $profile[Applicants::profile_has_education_background_degree] = [Applicants::narration => 'Degree Certificate Missing', Applicants::required => true] : '';
                    $setting->setting == ProductSettings::masters && $setting->value == ProductSettings::yes && empty(EducationBackground::searchEducations($this->applicant, EducationBackground::study_level_masters)) ? $profile[Applicants::profile_has_education_background_masters] = [Applicants::narration => 'Masters Certificate Missing', Applicants::required => true] : '';
                    $setting->setting == ProductSettings::phd && $setting->value == ProductSettings::yes && empty(EducationBackground::searchEducations($this->applicant, EducationBackground::study_level_phd)) ? $profile[Applicants::profile_has_education_background_phd] = [Applicants::narration => 'PhD Certificate Missing', Applicants::required => true] : '';
                    $setting->setting == ProductSettings::tuition_or_upkeep && in_array($setting->value, [ProductSettings::no, ProductSettings::both]) && (!is_object($applicant = Applicants::returnApplicant($this->applicant)) || (empty($applicant->account_number) && empty($applicant->smart_card_number))) ? $profile[Applicants::profile_has_bank] = [Applicants::narration => 'Bank Details Missing', Applicants::required => true] : '';
                    $setting->setting == ProductSettings::has_society_narration && $setting->value == ProductSettings::yes && (!is_object($institution = ApplicantsInstitution::forApplicant($this->applicant)) || empty($institution->narration)) ? $profile[Applicants::profile_has_institution_narration] = [Applicants::narration => 'Society Narration Missing', Applicants::required => true] : '';
                    $setting->setting == ProductSettings::employed && $setting->value == ProductSettings::yes && (!is_object(isset($applicant) ? $applicant : $applicant = Applicants::returnApplicant($this->applicant)) || $applicant->employed != LmBaseEnums::yesOrNo(LmBaseEnums::yes)->VALUE || !is_object(ApplicantsEmployment::forApplicant($this->applicant))) ? $profile[Applicants::profile_has_employment] = [Applicants::narration => 'Employment Details Missing', Applicants::required => true] : '';
                    $setting->setting == ProductSettings::guarantors && $setting->value > 0 && ($count = count(ApplicantsGuarantors::forApplicant($this->applicant))) < $setting->value ? $profile[Applicants::profile_has_guarantors] = [Applicants::narration => "$count Guarantor's Details Provided; $setting->value Required", Applicants::required => true] : '';
                    $setting->setting == ProductSettings::has_financial_literacy && $setting->value == ProductSettings::yes && !is_object(FinancialLiteracyScores::byApplicantAndAcademicYear($this->applicant, ProductOpening::returnOpening($this->application)->academic_year)) ? $profile[Applicants::application_has_financial_literacy] = [Applicants::narration => "Financial Literacy Status Not Upadated", Applicants::required => true] : '';
                }

        return empty($profile) || !is_array($profile) ? [] : $profile;
    }

    /**
     * 
     * @param boolean $appeal true - is appeal form
    ...
    public function printApplicationForm() {
        ...
        if ($filename = ProductOpening::applicationFormPrinter($this, false)) {
    ...
    public function printAppealForm() {
        ...
        if ($filename = ProductOpening::applicationFormPrinter($this, true)) {
